...
1. **Supabase 프로젝트 공유**: 새 프로젝트 만들지 않음. redirect URL만 추가
2. **모바일 고정 레이아웃**: `max-w-[430px]` — PC에서도 모바일 사이즈
3. **디자인 1:1 복제**: Flutter 앱과 동일한 색상/타이포/간격/라운딩
4. **Edge Function 재사용**: 사주/관상 분석 로직은 이미 서버에 있음
5. **결제 500원**: 카카오페이 간편결제, 서버사이드 검증 필수
6. **하위호환**: DB 스키마 변경 시 momo 네이티브 앱에 영향 없도록
