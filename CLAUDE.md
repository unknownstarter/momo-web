...
### 토큰 효율화 (MANDATORY)
- **응답은 짧고 핵심만.** 불필요한 설명, 반복, 요약 금지. 행동으로 보여줄 것.
- **동일한 파일을 반복 읽지 말 것.** 한번 읽은 내용은 기억하고 재활용.
- **에이전트 결과를 그대로 복붙하지 말 것.** 핵심만 추출해서 전달.
- **확인 질문을 최소화.** 맥락으로 판단 가능한 건 알아서 진행.
- **코드 블록 남발 금지.** 실제로 수정/생성하는 코드만 도구로 처리. 설명용 코드 블록은 최소화.
- **테이블/리스트 과다 사용 금지.** 정보 전달에 필요한 최소한만 사용.
...
> 새 디바이스에서 개발 시작 시 **[`docs/dev-setup.md`](docs/dev-setup.md)** 참조.
> Node 버전, 환경변수, 의존성, 카카오 OAuth, 토스페이먼츠 키 발급 등 전부 정리되어 있음.
...
| **결제** | 토스페이먼츠 (결제위젯 SDK v2) | 간편결제 (카카오페이, 토스페이 등) |
...
- `/payment-specialist` — 토스페이먼츠 연동, PG 심사, 결제 UX
