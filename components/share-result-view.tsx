      ...
      <div className="shrink-0 bg-hanji border-b border-hanji-border">
        <div className="flex">
          <button
            type="button"
            onClick={() => setTab("saju")}
            className={`flex-1 py-3 text-[15px] font-semibold border-b-2 transition-colors ${tab === "saju" ? "text-ink" : "text-ink-tertiary border-transparent"}`}
          ...
          <button
            ...
            className={`flex-1 py-3 text-[15px] font-semibold border-b-2 transition-colors ${tab === "gwansang" ? "text-ink" : "text-ink-tertiary border-transparent"}`}
      ...
      <div className="flex-1 min-h-0 overflow-y-auto overflow-x-hidden px-5 pt-6 pb-8">
        {hasNoData ? (
          <div className="rounded-2xl border border-hanji-border bg-hanji-elevated p-6 text-center">
            ...
            <p className="mt-2 text-sm text-ink-tertiary">분석이 완료되면 여기에 결과가 표시돼요.</p>
          </div>
        ) : (
          <>
