...
> **⚠️ 동성/이성 분기 (MANDATORY)**
> `generate-match-story`의 시스템 프롬프트가 "운명의 중매인", "로맨틱한 톤"으로 고정되어 있으므로, **동성 친구 궁합에서는 절대 호출하면 안 됨.** 동성 간에 "두 분은 운명적 인연..." 같은 텍스트가 나오면 유저 이탈 및 부정적 바이럴 발생.
>
> | 구분 | `calculate-compatibility` | `generate-match-story` | AI 스토리 표시 | 비용 |
> |------|--------------------------|----------------------|--------------|------|
> | **이성 (연인 궁합)** | 호출 | **호출** | 로맨틱 인연 스토리 표시 | ~3원 |
> | **동성 (친구 궁합)** | 호출 | **호출 금지** | 표시 안 함 | 0원 |
