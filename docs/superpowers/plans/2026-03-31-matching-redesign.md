...
Phase 1: `profiles`에서 `is_saju_complete = true`인 유저 수를 소셜 프루프로 표시.
Phase 2 (NHN Cloud 승인 + `matching_waitlist` 생성 후): "매칭 대기 이성 N명"으로 업그레이드.
...
## Task 5: matching-stats API + 새 /result 매칭 메인 페이지 작성

matching-stats API를 먼저 만들고, 기존 `/result/page.tsx`를 매칭 중심 메인으로 새로 작성한다.

**Files:**
- Create: `app/api/matching-stats/route.ts`
- Replace: `app/result/page.tsx` (기존 코드는 Task 2에서 detail로 복사 완료)

- [ ] **Step 0: matching-stats API 작성**

```tsx
// app/api/matching-stats/route.ts
